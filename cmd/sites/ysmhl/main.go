...
func main() {
	...
	doc, err = htmlquery.LoadURL("https://ysmhl.net/Seasons/Current/")

	groups := parseGroups(doc)
	log.Println(groups)

	var result = parser.FetchSchedules(SITE, "https://ysmhl.net/", "https://ysmhl.net/Groups/%s/Calendar/?Month=%d&Year=%d", groups, mm, yyyy)
