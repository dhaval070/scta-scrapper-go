...
func main() {
	...
	doc, err = htmlquery.LoadURL("https://threecountyhockey.ca/Seasons/Current/")

	groups := parseGroups(doc)
	log.Println(groups)

	var result = parser.FetchSchedules(SITE, "https://threecountyhockey.ca/", "https://threecountyhockey.ca/Groups/%s/Calendar/?Month=%d&Year=%d", groups, mm, yyyy)
