...
func main() {
	...
	doc, err = htmlquery.LoadURL("https://grandriverll.ca/Seasons/Current/")

	groups := parseGroups(doc)
	log.Println(groups)

	var result = parser.FetchSchedules(SITE, "https://grandriverll.ca/", "https://grandriverll.ca/Groups/%s/Calendar/?Month=%d&Year=%d", groups, mm, yyyy)
