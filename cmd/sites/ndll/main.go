...
func main() {
	...
	doc, err = htmlquery.LoadURL("https://ndll.ca/Seasons/Current/")

	groups := parseGroups(doc)
	log.Println(groups)

	var result = parser.FetchSchedules(SITE, "https://ndll.ca/", "https://ndll.ca/Groups/%s/Calendar/?Month=%d&Year=%d", groups, mm, yyyy)
