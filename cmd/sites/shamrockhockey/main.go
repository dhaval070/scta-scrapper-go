...
func main() {
	...
	doc, err = htmlquery.LoadURL("https://shamrockhockeyleague.ca/Seasons/Current/")

	groups := parseGroups(doc)
	log.Println(groups)

	var result = parser.FetchSchedules(SITE, "https://shamrockhockeyleague.ca/", "https://shamrockhockeyleague.ca/Groups/%s/Calendar/?Month=%d&Year=%d", groups, mm, yyyy)
