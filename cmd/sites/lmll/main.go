...
func main() {
	...
	doc, err = htmlquery.LoadURL("https://lmll.ca/Seasons/Current/")

	groups := parseGroups(doc)
	log.Println(groups)

	var result = parser.FetchSchedules(SITE, "https://lmll.ca/", "https://lmll.ca/Groups/%s/Calendar/?Month=%d&Year=%d", groups, mm, yyyy)
