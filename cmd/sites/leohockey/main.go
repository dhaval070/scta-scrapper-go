...
func main() {
	...
	doc, err = htmlquery.LoadURL("https://leohockey.ca/Seasons/Current/")

	groups := parseGroups(doc)
	log.Println(groups)

	var result = parser.FetchSchedules(SITE, "https://leohockey.ca/", "https://leohockey.ca/Groups/%s/Calendar/?Month=%d&Year=%d", groups, mm, yyyy)
